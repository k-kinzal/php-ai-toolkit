...
final class PhpUnitMockApiRule implements Rule
{
    ...
    private function processCall(?string $methodName, ?\PhpParser\Node\Expr $firstArg, int $line, Scope $scope): array
    {
        ...
        if (in_array($methodName, self::ALWAYS_PROHIBITED_METHODS, true)) {
            return [$this->buildProhibitedApiError($methodName, $line)];
        }
        ...
        if ($firstArg === null) {
            return [$this->buildRequiresLiteralInterfaceError($methodName, $line)];
        }
        ...
        if (!$this->reflectionProvider->hasClass($targetTypeName)) {
            return [$this->buildRequiresInterfaceError($methodName, $targetTypeName, $line)];
        }
    ...
    private function buildProhibitedApiError(string $methodName, int $line): IdentifierRuleError
    {
    ...
    private function buildRequiresLiteralInterfaceError(string $methodName, int $line): IdentifierRuleError
    {
        return RuleErrorBuilder::message(
            sprintf(
    ...
    private function buildRequiresInterfaceError(string $methodName, string $targetTypeName, int $line): IdentifierRuleError
    {
        return RuleErrorBuilder::message(
            sprintf(
