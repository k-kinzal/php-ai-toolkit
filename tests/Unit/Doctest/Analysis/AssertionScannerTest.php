...
final class AssertionScannerTest extends TestCase
{
    ...
    public function testScanLineIgnoresASlashPairInsideAStringLiteral(): void
    {
        $line = (new AssertionScanner())->scanLine('$url = "https://example.com/a // b";');

        self::assertSame('$url = "https://example.com/a // b";', $line->code);
        self::assertNull($line->marker);
    }

    public function testTrailingCommentReportsTheCommentAndWhereItStarts(): void
    {
        $scanner = new AssertionScanner();

        self::assertSame(['text' => '// => 3', 'offset' => 7], $scanner->trailingComment('  $sum // => 3'));
        self::assertNull($scanner->trailingComment('$url = "https://example.com";'));
        self::assertNull($scanner->trailingComment('$sum = 1;'));
    }
