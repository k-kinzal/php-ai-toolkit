...
To express "every module directory must contain exactly its manifest", combine `require` with `allow`:
...
The limit value itself is allowed: a rule with `max_files: 15` accepts 15 files
and reports the 16th.
