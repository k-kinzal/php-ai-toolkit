...
final class AiRulesErrorFormatter implements ErrorFormatter
{
    ...
    private function formatForHuman(AnalysisResult $analysisResult, Output $output): int
    {
        ...
        $fileErrors = $this->groupFileErrors($analysisResult->getFileSpecificErrors());
        $fileCount = count($fileErrors);

        foreach ($fileErrors as $file => $errors) {
            $this->writeHumanFileErrors($output, $file, $errors);
        }
        ...
        $message = $this->buildHumanSummaryMessage($totalErrors, $warningCount, $fileCount);
        $this->writeHumanSummary($output, $totalErrors, $warningCount, $message);
     ...
     */
    private function groupFileErrors(array $errors): array
    {
        $fileErrors = [];
     ...
     * @param list<Error> $errors
     */
    private function writeHumanFileErrors(Output $output, string $file, array $errors): void
    {
    ...
    private function buildHumanSummaryMessage(int $totalErrors, int $warningCount, int $fileCount): string
    {
        $message = sprintf(
            'Found %d %s%s',
            $totalErrors,
    ...
    private function writeHumanSummary(Output $output, int $totalErrors, int $warningCount, string $message): void
    {
        if ($totalErrors > 0) {
